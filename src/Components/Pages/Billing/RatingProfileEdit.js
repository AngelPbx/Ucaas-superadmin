...
function RatingProfileEdit() {
    const location = useLocation()
    const locationData = location.state
    const navigate = useNavigate()
    const [Tenant,setTenant]=useState(locationData.Tenant)
    const [Category,setCategory]=useState(locationData.Category)
    const [Subject,setSubject]=useState(locationData.Subject)
    const [RatingPlanId,setRatingPlanId]=useState(locationData.RatingPlanId)
    const [RatesFallbackSubject,setRatesFallbackSubject]=useState(locationData.RatesFallbackSubject)
    ...
    async function handleSubmit(){
        if(Tenant===""){
            ...
            const apiData = await generalPutFunction(`/rating-profile/update/${locationData.id}`,parsedData)
            if(apiData.status){
                setLoading(false)
...
export default RatingProfileEdit
