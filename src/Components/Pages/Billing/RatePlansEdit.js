...
function RatePlansEdit() {
    const location = useLocation();
    const locationData = location.state;
    const navigate = useNavigate()
    const [name,setName]=useState(locationData.name)
    const [DestinationRatesId,setDestinationRatesId]=useState(locationData.DestinationRatesId)
    const [TimingTag,setTimingTag]=useState(locationData.TimingTag)
    const [Weight,setWeight]=useState(locationData.Weight)
    ...
    async function handleSubmit(){
        if(name===""){
            ...
            const apiData = await generalPutFunction(`/rating-plan/update/${locationData.id}`,parsedData)
            if(apiData.status){
                setLoading(false)
