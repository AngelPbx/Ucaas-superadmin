...
function BillingRateEdit() {
    const location = useLocation();
    const locationData = location.state;
    const navigate = useNavigate()
    const [name,setName]=useState(locationData.name)
    const [ConnectFee,setConnectFee]=useState(locationData.ConnectFee)
    const [Rate,setRate]=useState(locationData.Rate)
    const [RateUnit,setRateUnit]=useState(locationData.RateUnit)
    const [RateIncrement,setRateIncrement]=useState(locationData.RateIncrement)
    const [GroupIntervalStart,setGroupIntervalStart]=useState(locationData.GroupIntervalStart)
    ...
    async function handleSubmit(){
        if(name===""){
            ...
            const apiData = await generalPutFunction(`/rate/update/${locationData.id}`,parsedData)
            if(apiData.status){
                setLoading(false)
            ...
            <div className="col-12" id="subPageHeader">
              <div className="row px-xl-3">
                <div className="col-xl-9 my-auto">
                  <h4 className="my-auto">Update Destination Rate</h4>
...
export default BillingRateEdit
